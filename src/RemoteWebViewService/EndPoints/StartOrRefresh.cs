    ...
    public static partial class Endpoints
    {
        public static RequestDelegate StartOrRefresh()
        {
            return async context =>
            {
                ...
                if (!serviceDictionary.TryGetValue(guid.ToString(), out var serviceState))
                {
                ...
                if (!serviceState.Refresh)
                {
                    ...
                    FileStats.Update(serviceDictionary, guid.ToString(), fileInfo);
        ...
        private static async Task<bool> WaitForClientShutdown(ConcurrentDictionary<string, ServiceState> serviceDictionary, string guid, TimeSpan timeout, TimeSpan delay)
        {
