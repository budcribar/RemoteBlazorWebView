    ...
    public class Worker
    {
        ...
        public async Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Attempting to watch file: {FilePath}", _fileToWatch);
                ...
                _logger.LogInformation("Started watching file: {FilePath}", _fileToWatch);
                    ...
                    switch (response.ResponseCase)
                    {
                        case WatchFileResponse.ResponseOneofCase.Notification:
                            ...
                            if (!string.IsNullOrEmpty(response.Notification.RunArguments))
                            {
                                ...
                                _logger.LogInformation("Run arguments updated to: {RunArguments}", currentRunArguments);
                            }
                            ...
                            try
                            {
                                ...
                                isStreaming = true;
                                _logger.LogInformation("Created temp file at: {TempFilePath}", _tempFilePath);
                            }
                            catch (Exception ex)
                            {
                                _logger.LogError(ex, "Failed to create temp file at {TempFilePath}", _tempFilePath);
                            }
                            break;
                            ...
                            if (isStreaming && fileStream != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        _logger.LogInformation("Received and wrote a {ChunkSizeKB}KB chunk.", bytes.Length / 1024);
                                    }
                                    catch (Exception ex)
                                    {
        ...
        private void ExecuteFile(string filePath, string arguments)
        {
            try
            {
                ...
                _logger.LogInformation("Executing file: {FilePath} with arguments: {Arguments}", filePath, arguments);
                ...
                {
                    _logger.LogInformation("Successfully started process ID: {ProcessId}, Name: {ProcessName}", processStart.Id, processStart.ProcessName);
                }
                else
                {
                    _logger.LogWarning("Failed to start the process for file: {FilePath}", filePath);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            _logger.LogInformation("Checking for existing processes named: {ProcessName}", processName);
            ...
            foreach (var process in existingProcesses)
            {
                try
                {
                    _logger.LogInformation("Attempting to close process ID: {ProcessId}, Name: {ProcessName}", process.Id, process.ProcessName);
                    ...
                    if (process.CloseMainWindow())
                    {
                        ...
                        {
                            _logger.LogWarning("Process ID: {ProcessId} did not exit gracefully. Attempting to kill.", process.Id);
                            process.Kill(); // Forcefully terminate the process
                            process.WaitForExit(); // Wait indefinitely for the process to exit
                            _logger.LogInformation("Process ID: {ProcessId} has been forcefully terminated.", process.Id);
                        }
                        else
                        {
                            _logger.LogInformation("Process ID: {ProcessId} has exited gracefully.", process.Id);
                        }
                    }
                    else
                    {
                        _logger.LogWarning("Process ID: {ProcessId} does not have a main window or could not receive the close message. Attempting to kill.", process.Id);
                        process.Kill(); // Forcefully terminate the process
                        process.WaitForExit(); // Wait indefinitely for the process to exit
                        _logger.LogInformation("Process ID: {ProcessId} has been forcefully terminated.", process.Id);
                    }
                ...
                {
                    _logger.LogError(ex, "Error while attempting to terminate process ID: {ProcessId}", process.Id);
                }
            }
        }
                ...
                using var results = searcher.Get();
                {
                    foreach (ManagementObject mo in results.Cast<ManagementObject>())
                    {
                        try
                        {
                            ...
                            _logger.LogInformation("Found child process: ID={ProcessId}, Name={ProcessName}", childProcess.Id, childProcess.ProcessName);
                        ...
                        catch (ArgumentException)
                        {
                            ...
                            _logger.LogWarning("Process with ID {ProcessId} no longer exists.", mo["ProcessId"]);
                        }
                    }
                }
            }
        ...
        private void TerminateProcess(Process process)
        {
            try
            {
                _logger.LogInformation("Attempting to terminate child process ID: {ProcessId}, Name: {ProcessName}", process.Id, process.ProcessName);
                ...
                if (process.CloseMainWindow())
                {
                    if (!process.WaitForExit(5000))
                    {
                        _logger.LogWarning("Child process ID: {ProcessId} did not exit gracefully. Attempting to kill.", process.Id);
                        process.Kill();
                        process.WaitForExit();
                        _logger.LogInformation("Child process ID: {ProcessId} has been forcefully terminated.", process.Id);
                    }
                    else
                    {
                        _logger.LogInformation("Child process ID: {ProcessId} has exited gracefully.", process.Id);
                    }
                }
                else
                {
                    _logger.LogWarning("Child process ID: {ProcessId} does not have a main window or could not receive the close message. Attempting to kill.", process.Id);
                    process.Kill();
                    process.WaitForExit();
                    _logger.LogInformation("Child process ID: {ProcessId} has been forcefully terminated.", process.Id);
                }
            ...
            {
                _logger.LogError(ex, "Error while attempting to terminate child process ID: {ProcessId}", process.Id);
            }
        }
    }
}
